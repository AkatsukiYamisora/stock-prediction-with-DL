...
@version: 3.8.3
@author: Yamisora
@file: stock_indicator.py
股票技术指标接口
Created on 2018/07/26
@original author: Wangzili
...
def wr(df, n=14):
    ...
    _wr['w_r'] = (higest - df.close) / (higest - df.low.rolling(n).min()) * 100
    return _wr
...
def env(df, n=14):
    ...
    _env = pd.DataFrame()
    _env['date'] = df.date
    _env['env_up'] = df.close.rolling(n).mean() * 1.06
    _env['env_low'] = df.close.rolling(n).mean() * 0.94
    return _env
...
def vmacd(df, qn=12, sn=26, m=9):
    ...
    _vmacd['vmacd'] = (_vmacd['diff'] - _vmacd['dea'])
    del _vmacd['diff'], _vmacd['dea']
    return _vmacd
...
def vosc(df, n=12, m=26):
    ...
    _c = pd.DataFrame()
    _c['date'] = df['date']
    _c['vosc'] = (_ma(df.volume, n) - _ma(df.volume, m)) / _ma(df.volume, n) * 100
    return _c
...
def zdzb(df, n=125, m=5, k=20):
    ...
    p[p < 0] = np.nan
    q[q >= 0] = np.nan
    _zdzb['zdzb_a'] = p.rolling(n).count() / q.rolling(n).count()
    _zdzb['zdzb_b'] = _zdzb.zdzb_a.rolling(m).mean()
    _zdzb['zdzb_d'] = _zdzb.zdzb_a.rolling(k).mean()
    return _zdzb
...
# if __name__ == "__main__":
    ...
    # data = pro.daily(ts_code='000001.SZ', start_date='20211101', end_date='20211128')
